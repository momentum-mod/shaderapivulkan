...
//-----------------------------------------------------------------------------
class CShaderDeviceMgrVk : public CShaderDeviceMgrBase
{
...
extern CShaderDeviceMgrVk* g_pShaderDeviceMgrVk;
...
inline VkInstance VkInst()
{
	return g_pShaderDeviceMgrVk->GetInstance();
}
...
//-----------------------------------------------------------------------------
// The Vk implementation of the shader device
//-----------------------------------------------------------------------------
class CShaderDeviceVk : public CShaderDeviceBase
{
	// Methods of IShaderDevice
public:
...
protected:
	...
	bool CreateVkDevice(void* pHWnd, int nAdapter, const ShaderDeviceInfo_t &info);

	// Actually creates the D3D Device once the present parameters are set up
	VkPhysicalDevice InvokeCreateDevice(void* hWnd, int nAdapter, uint32_t deviceCreationFlags);

	// Checks for CreateQuery support
	void DetectQuerySupport(VkDevice pD3DDevice);
...
extern CShaderDeviceVk* g_pShaderDeviceVk;
...
inline CShaderDeviceVk::CShaderDeviceVk()
{
}
...
inline void CShaderDeviceVk::OtherAppInitializing(bool initializing)
{
}
